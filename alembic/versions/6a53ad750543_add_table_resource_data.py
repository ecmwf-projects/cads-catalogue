...
def upgrade() -> None:
    alembic.op.create_table(
        ...
        sa.Column(
            "resource_uid",
            sa.String,
            sa.ForeignKey("resources.resource_uid"),
            nullable=False,
        ),
    )
    alembic.op.create_unique_constraint(
        "resource_uid_uc", "resource_data", ["resource_uid"]
    )
    sql = (
...
def downgrade() -> None:
    alembic.op.add_column(
        "resources", sa.Column("adaptor_configuration", dialect_postgresql.JSONB)
    )
    alembic.op.add_column(
        "resources", sa.Column("constraints_data", dialect_postgresql.JSONB)
    )
