...
try:
    ...
    #  cads-e2e-tests not wanted in retrieve-api, cannot process sanity checks
    pass
...
def report2sanity_check(report):
...
def update_dataset_sanity_check(session_obj, dataset_uid, report, retain_only):
    ...
    logger.info(f"sanity check outcome stored for {dataset_uid!r}.")


def update_sanity_checks_by_file(session_obj, report_path, retain_only):
